...
least_samples = 30 # guarantee that each client must have at least one samples for testing. 
alpha = 100 # for Dirichlet distribution
...
# def getDataloader(self,is_train = True):
        
#     idx_count = self.getidx_num(is_train)
#     all_data = []
#     for idx in range(idx_count):
#         client_data = read_client_data(self.dataset, idx, is_train)
#         all_data.extend(client_data)
#     return DataLoader(all_data, self.batch_size, drop_last=False, shuffle=True)
    

def getidx_num(dataset, is_train = True):
    # 要统计的目录路径
    if is_train:
        directory_path = os.path.join(dataset, 'train/')
    else:
        directory_path = os.path.join(dataset, 'test/')
    # 使用 os.listdir 函数列出目录下的所有文件和子目录
